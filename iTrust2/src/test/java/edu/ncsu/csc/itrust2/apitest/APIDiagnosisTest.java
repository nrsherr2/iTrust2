...
public class APIDiagnosisTest {
	...
	public void testDiagnoses() throws UnsupportedEncodingException, Exception {
		/*
		 * Create a HCP, admin and a Patient to use. If they already exist, this will do
		 * nothing
		 */
		final UserForm hcp = new UserForm("hcp", "123456", Role.ROLE_HCP, 1);
		mvc.perform(post("/api/v1/users").contentType(MediaType.APPLICATION_JSON).content(TestUtils.asJsonString(hcp)));

		final UserForm admin = new UserForm("admin", "123456", Role.ROLE_ADMIN, 1);
		mvc.perform(
				post("/api/v1/users").contentType(MediaType.APPLICATION_JSON).content(TestUtils.asJsonString(admin)));

		final UserForm patient = new UserForm("patient", "123456", Role.ROLE_PATIENT, 1);
		mvc.perform(
				post("/api/v1/users").contentType(MediaType.APPLICATION_JSON).content(TestUtils.asJsonString(patient)));
		...
		// are there
		content = mvc.perform(get("/api/v1/diagnosesforvisit/" + visit.getId()).contentType(MediaType.APPLICATION_JSON)
				.content(TestUtils.asJsonString(codeForm))).andReturn().getResponse().getContentAsString();
		...
		// are there
		// CUSTOM
		content = mvc.perform(get("/api/v1/diagnosis/patient/" + "patient").contentType(MediaType.APPLICATION_JSON)
				.content(TestUtils.asJsonString(codeForm))).andReturn().getResponse().getContentAsString();
